...
28. `fix-rls-encaissements.sql` (2026-04-21, **réécrit 2026-04-25**) — active RLS FORCE sur `encaissements` + 4 policies (SELECT/INSERT/UPDATE filtrés sur `consultant_id = get_current_consultant_id()` OR fallback via `dossier → clients.consultant_id` OR `is_manager()` OR `is_back_office()`, DELETE managers-only). La V1 du script joignait sur `client_id` inexistant → réécrit avec le bon schéma. **Appliqué en prod 2026-04-25** via Supabase SQL editor, succès confirmé. V1 sécurité débloquée.
...
9. ~~**`fix-rls-encaissements.sql`**~~ → **DONE 2026-04-25** : script réécrit (la V1 joignait sur `client_id` inexistant, nouvelle version utilise `consultant_id` direct + fallback via `dossier → clients.consultant_id`). **Appliqué en prod 2026-04-25 via Supabase SQL editor**, succès confirmé Maxine. RLS désormais activée + forcée sur `encaissements` avec 4 policies (select/insert/update/delete). V1 sécurité débloquée.
