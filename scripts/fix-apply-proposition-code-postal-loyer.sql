-- =====================================================
-- FEATURE : extension RPC KYC pour `code_postal` + `montant_loyer`
-- Date : 2026-04-22
-- Context : étape 1 de l'audit KYC (frictions création fiche client).
--           On a ajouté 2 colonnes nullable dans `clients`
--           (cf. add-client-etape1-frictions.sql). Pour que le portail
--           public KYC puisse les soumettre et que la revue interne puisse
--           les merger, il faut :
--             1. Les whitelister dans `_kyc_editable_fields()` (sinon la
--                boucle FOREACH ignore la clé silencieusement).
--             2. Les brancher dans le UPDATE CASE de `kyc_apply_proposition`
--                (avec le bon cast : TEXT pour code_postal, NUMERIC pour
--                montant_loyer).
--
-- FIX
-- ---
-- On remplace intégralement les 2 fonctions par CREATE OR REPLACE, en
-- gardant exactement la même signature et tout le corps inchangé hors
-- des 2 ajouts. La version repartie est celle produite par le patch
-- `fix-apply-proposition-enfants-details.sql` (enfants_details TEXT),
...
--    (corps identique à fix-apply-proposition-enfants-details.sql, avec
--    2 lignes ajoutées pour code_postal + montant_loyer)
-- ---------------------------------------------------------------------
...
-- Smoke tests :
-- 1. whitelist contient bien les 2 nouvelles clés
--    SELECT 'code_postal'   = ANY(public._kyc_editable_fields());  -- t
--    SELECT 'montant_loyer' = ANY(public._kyc_editable_fields());  -- t
--
