-- =====================================================
-- FIX : RPC kyc_apply_proposition — restauration du bloc
--       de propagation des champs signature sur `clients`
-- Date : 2026-04-24
--
-- Contexte
-- --------
-- Le fix `fix-apply-proposition-nom-jeune-fille.sql` (2026-04-24) a ajouté
-- `nom_jeune_fille` et `patrimoine_professionnel` à la whitelist et au
-- UPDATE CASE des champs scalaires. Mais il a, par omission, SUPPRIMÉ le
-- bloc qui propageait les champs signature (`kyc_signed_at`, `kyc_signer_name`,
-- `kyc_signer_ip`, `kyc_completion_rate`, `kyc_incomplete_signed`,
-- `kyc_missing_fields`, `kyc_consent_accuracy`, `kyc_consent_incomplete`)
-- depuis la proposition vers la table `clients`.
--
-- Symptôme observé (2026-04-24) : Maxine signe son KYC via le portail,
-- la proposition atteint `status='fully_applied'` après acceptation
-- consultant, les emails post-signature partent correctement (consultant
-- + client reçoivent le PDF), MAIS `clients.kyc_signed_at` reste NULL et
-- donc :
--   · Le badge de statut KYC reste bloqué sur « En cours »
--   · La timeline 3-points (chantier 2 étape 3) affiche 2/3 cochés
--   · La bannière de signature incomplète ne s'affiche pas
--
-- Diagnostic confirmé par SELECT direct : la proposition est
-- `fully_applied`, signed_at/signer_name/completion_rate sont bien
-- renseignés côté `kyc_propositions`, mais rien n'est remonté sur
-- `clients`.
--
-- Fix
-- ---
-- CREATE OR REPLACE sur la RPC en repartant de l'état
-- `fix-apply-proposition-nom-jeune-fille.sql` et en ajoutant le bloc
-- manquant AVANT le RETURN. Différence avec la version pre-fix : on
-- propage les champs signature dès qu'il y a eu AU MOINS une
-- acceptation (accepted_keys non vide), pas seulement sur fully_applied.
-- Argument : le client A signé c'est un fait juridique ;
-- `kyc_signed_at` doit refléter l'acte de signature indépendamment du
-- fait que le consultant ait accepté 100% ou 80% des changements. Le
-- filtre `WHERE c.kyc_signed_at IS NULL` garantit l'idempotence — on
-- n'écrase jamais une signature antérieure.
--
...
BEGIN
  ...
  -- =====================================================================
  -- FIX 2026-04-24 : restauration de la propagation des champs signature
  -- --------------------------------------------------------------------
  -- On propage dès qu'il y a eu AU MOINS une acceptation (donc
  -- final_status IN ('fully_applied','partially_applied')). Ces champs
  -- décrivent l'ACTE de signature côté client — ils restent vrais
  -- indépendamment du taux d'acceptation côté consultant. Le filtre
  -- `kyc_signed_at IS NULL` empêche d'écraser une signature antérieure
  -- si la même proposition était appliquée plusieurs fois (la RPC
  -- refuse déjà les non-pending plus haut, mais ceinture + bretelles).
  -- =====================================================================
...
-- Smoke test (commenté)
-- =====================================================================
-- SELECT proname, prosrc LIKE '%kyc_signed_at%' AS has_signature_block
