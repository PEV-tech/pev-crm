-- =====================================================
-- FEATURE : extension RPC KYC pour `nom_jeune_fille`
--           + whitelist `patrimoine_professionnel` en lecture.
-- Date : 2026-04-24
-- Référence: Corrections app Étape 4 point 4.4 (plan 2026-04-24)
--
-- Context
-- -------
-- Colonne `clients.nom_jeune_fille` créée depuis add-kyc-fields.sql
-- (2026-04-16) mais jamais whitelistée côté RPC. Conséquence : le portail
-- public KYC peut collecter la valeur dans le formulaire mais la
-- proposition soumise ignore silencieusement la clé à l'apply côté
-- consultant — le client avait l'impression d'avoir renseigné son nom
-- de jeune fille, il ne l'était pas en base.
--
-- On en profite pour whitelister aussi `patrimoine_professionnel`
-- (ajoutée en 1.6, cf. add-patrimoine-professionnel.sql) afin que la
-- section remonte côté diff du consultant même si le portail public ne
-- l'édite pas encore (elle est READONLY_JSONB_KEYS côté front).
--
-- Fix
-- ---
-- CREATE OR REPLACE x2 sur le dernier état en date (patch
-- `fix-apply-proposition-code-postal-loyer.sql`, 2026-04-22), avec
-- ajout des 2 clés + 2 branches CASE.
--
-- Idempotent : CREATE OR REPLACE x2.
-- =====================================================
...
--    (corps identique à fix-apply-proposition-code-postal-loyer.sql,
--     avec 2 branches supplémentaires pour nom_jeune_fille et
...
-- Dépendance ordre d'exécution : ce patch doit être joué APRÈS
-- add-patrimoine-professionnel.sql (sinon la colonne cible n'existe pas).

-- Smoke tests :
-- 1. whitelist contient bien nom_jeune_fille + patrimoine_professionnel
--    SELECT 'nom_jeune_fille'          = ANY(public._kyc_editable_fields()); -- t
--    SELECT 'patrimoine_professionnel' = ANY(public._kyc_editable_fields()); -- t
--
-- 2. version RPC appliquée contient bien les 2 nouvelles branches CASE
--    SELECT prosrc FROM pg_proc WHERE proname = 'kyc_apply_proposition'
--      AND prosrc LIKE '%''nom_jeune_fille''%'
--      AND prosrc LIKE '%''patrimoine_professionnel''%';
--    Doit matcher.
